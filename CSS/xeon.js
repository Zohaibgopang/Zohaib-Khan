...
require('./settings')
const { default: zohaibBotIncConnect, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
...
async function startXeonBotInc() {
    ...
    store.bind(zohaib.ev)
    
    // anticall auto block
    zohaib.ws.on('CB:call', async (json) => {
    const callerId = json.content[0].attrs['call-creator']
    if (json.content[0].tag == 'offer') {
    let xeonfek = await zohaib.sendContact(callerId, global.owner)
    zohaib.sendMessage(callerId, { text: `Automatic Block System!\nDon't Call Bot!\nPlease Ask Or Contact The Owner To Unblock You!`}, { quoted : xeonfek })
    await sleep(8000)
    await XeonBotInc.updateBlockStatus(callerId, "block")
    }
    })

    zohaib.ev.on('messages.upsert', async chatUpdate => {
        ...
        if (!Zohaib.public && !mek.key.fromMe && chatUpdate.type === 'notify') return
        if (mek.key.id.startsWith('BAE5') && mek.key.id.length === 16) return
        m = smsg(zohaibkhan, mek, store)
        require("./XeonCheems4")(zohaibkhan, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
